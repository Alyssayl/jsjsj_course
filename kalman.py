...
class Ui_MainWindow(object):
    def __init__(self, MainWindow):
        ...
        self._setupUi()
        # 控件分组
        self.bg_edit = [self.bt_brightening, self.bt_whitening, self.bt_big_eyes, self.bt_thin_face, self.bt_sharpen, self.bt_smooth]
        ...
        self.sls = [self.sl_brightening, self.sl_sharpen, self.sl_whitening, self.sl_thin_face, self.sl_big_eyes, self.sl_smooth]
        # 用于显示图片的标签
        ...
        if os.path.exists("./model/shape_predictor_68_face_landmarks.dat"):
            self.path_predictor = os.path.abspath("./model/shape_predictor_68_face_landmarks.dat")
        else:
    ...
    def _set_connect(self):
        ...
        for op in ['sharpen', 'whitening', 'smooth', 'brightening', 'cancel', 'confirm', 'reset', 'save',
                   'save_compare', 'view_compare', 'big_eyes', 'thin_face']:
    ...
    # 大眼程度
    def _big_eyes(self):
        value = min(1, max(self.sl_big_eyes.value() / 200, 0))
        print(value)

        def fun(face, v):
            face.organs['left eye'].big_eyes(value, confirm=False)
            face.organs['right eye'].big_eyes(value, confirm=False)
            face.organs['left brow'].big_eyes(value, confirm=False)
            face.organs['right brow'].big_eyes(value, confirm=False)
            face.organs['nose'].big_eyes(value, confirm=False)
            face.organs['forehead'].big_eyes(value, confirm=False)
            face.organs['mouth'].big_eyes(value, confirm=False)
            face.big_eyes(value, confirm=False)

        self._mapfaces(fun, value)

    # 瘦脸程度
    def _thin_face(self):
        value = min(1, max(self.sl_thin_face.value() / 200, 0))
        print(value)
    ...
    def _setupUi(self):
        ...
        # 美白
        self.bt_whitening = QtWidgets.QPushButton(self.centralWidget)  # 美白按钮
        self.bt_whitening.setObjectName("bt_whitening")
        self.gridLayout.addWidget(self.bt_whitening, 0, 0, 1, 1)
        self.sl_whitening = QtWidgets.QSlider(self.centralWidget)  # 美白滑动条
        ...
        # 亮眼:big_eyes
        self.bt_big_eyes = QtWidgets.QPushButton(self.centralWidget)
        self.bt_big_eyes.setObjectName("bt_whitening")
        self.gridLayout.addWidget(self.bt_big_eyes, 8, 0, 1, 1)
        self.sl_big_eyes = QtWidgets.QSlider(self.centralWidget)
        self.sl_big_eyes.setOrientation(QtCore.Qt.Horizontal)
        self.sl_big_eyes.setObjectName("sl_whitening")
        self.gridLayout.addWidget(self.sl_big_eyes, 8, 1, 1, 1)
        # spacerItem = QtWidgets.QSpacerItem(40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum)
        # self.gridLayout.addItem(spacerItem, 0, 2, 1, 1)

        # 瘦脸:thin_face
        self.bt_thin_face = QtWidgets.QPushButton(self.centralWidget)
        self.bt_thin_face.setObjectName("bt_whitening")
        self.gridLayout.addWidget(self.bt_thin_face, 9, 0, 1, 1)
        self.sl_thin_face = QtWidgets.QSlider(self.centralWidget)
        self.sl_thin_face.setOrientation(QtCore.Qt.Horizontal)
        self.sl_thin_face.setObjectName("sl_whitening")
        self.gridLayout.addWidget(self.sl_thin_face, 9, 1, 1, 1)
        # spacerItem = QtWidgets.QSpacerItem(40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum)
        # self.gridLayout.addItem(spacerItem, 0, 2, 1, 1)
    ...
    def retranslateUi(self):
        ...
        self.bt_big_eyes.setText(_translate("MainWindow", "大眼"))
